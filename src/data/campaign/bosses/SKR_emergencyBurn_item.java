 ...
 * @author Tartiflette
 */
public class SKR_emergencyBurn_item extends BaseSpecialItemPlugin{
	...
	public void performRightClickAction() {
                ...
                if (player.getAbilities().contains(SKR_ids.ABILITY_EMERGENCY_BURN)) return;

                // Unlock the ability and replace the corresponding vanilla ability
                player.addAbility(SKR_ids.ABILITY_EMERGENCY_BURN);
                
                boolean slotted = false;
                for (AbilitySlotAPI slot : Global.getSector().getUIData().getAbilitySlotsAPI().getCurrSlotsCopy()){
                    if (slot.getAbilityId() != null && slot.getAbilityId().equals("emergency_burn")){
                        slot.setAbilityId(SKR_ids.ABILITY_EMERGENCY_BURN);
                        slotted=true;
                        break;
                    }
                }
