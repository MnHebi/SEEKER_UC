...
public class SKR_rampageLoot implements FleetEventListener{
    
    private final String RAMP_DROP_ALREADY = "$ramp_drop";
    
    @Override
    public void reportBattleOccurred(CampaignFleetAPI fleet, CampaignFleetAPI primaryWinner, BattleAPI battle) {
        
        // ignore that whole ordeal if the ship already dropped
        if(Global.getSector().getMemoryWithoutUpdate().contains(RAMP_DROP_ALREADY) 
                && Global.getSector().getMemoryWithoutUpdate().getBoolean(RAMP_DROP_ALREADY)){
            return;            
        }
        
        if(fleet.getFlagship()==null || !fleet.getFlagship().getHullSpec().getBaseHullId().equals("SKR_rampage")){
            //boss is dead, spawn a derelict            
            SectorEntityToken wreck;
            if(fleet.isInHyperspace()){
                //in hyperspace, just leave it there
                wreck = Global.getSector().getHyperspace().addCustomEntity(SKR_ids.BOSS_RAMPAGE_WRECK, txt("plague_B_wreck"), SKR_ids.BOSS_RAMPAGE_WRECK, Factions.NEUTRAL);
                ...
                wreck = system.addCustomEntity(SKR_ids.BOSS_RAMPAGE_WRECK, txt("plague_B_wreck"), SKR_ids.BOSS_RAMPAGE_WRECK, Factions.NEUTRAL);
            ...
            MarketAPI wreckMarket = Global.getFactory().createMarket(SKR_ids.BOSS_RAMPAGE_MARKET, txt("plague_B_wreckMarket"), 0);
...
//            wreck.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addHullmods("SKR_plagueLPC", 1);
            wreck.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addCommodity(Commodities.ALPHA_CORE, 2);
            wreck.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addSpecial(new SpecialItemData(SKR_ids.BOSS_RAMPAGE_ITEM, null), 1);
            ...
            Global.getSector().getMemory().set(RAMP_DROP_ALREADY,true);            
            Global.getSector().getMemory().set("$SKR_rampage_boss",false);           
//            Global.getSector().getMemory().set("$SKR_rampage",true); 
