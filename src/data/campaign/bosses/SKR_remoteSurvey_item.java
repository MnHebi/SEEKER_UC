 ...
 * @author Tartiflette
 */
public class SKR_remoteSurvey_item extends BaseSpecialItemPlugin{
	...
	public void performRightClickAction() {
                ...
                if (player.getAbilities().contains(SKR_ids.ABILITY_REMOTE_SURVEY)) return;

                // Unlock the ability and replace the corresponding vanilla ability
                player.addAbility(SKR_ids.ABILITY_REMOTE_SURVEY);
                
                boolean slotted = false;
                for (AbilitySlotAPI slot : Global.getSector().getUIData().getAbilitySlotsAPI().getCurrSlotsCopy()){
                    if (slot.getAbilityId() != null && slot.getAbilityId().equals("remote_survey")){
                        slot.setAbilityId(SKR_ids.ABILITY_REMOTE_SURVEY);
                        slotted=true;
                        break;
                    }
                }
