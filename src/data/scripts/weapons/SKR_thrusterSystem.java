...
public class SKR_thrusterSystem implements EveryFrameWeaponEffectPlugin {
    ...
    private float TURN_RIGHT_ANGLE=0, THRUST_TO_TURN=0, NEUTRAL_ANGLE=0, FRAMES=0;
    //sprite size, could be scaled with the engine width to allow variable engine length
    private Vector2f size= new Vector2f(8,100);
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        */
        ...
        float systemEffect=SYSTEM.getEffectLevel();
        if(systemEffect==0){
...
//                VectorUtils.rotate(offset, -SHIP.getFacing(), offset);
                ...
                if(!turn){
                    //thrust only, easy.
                    thrust(weapon, accelerateAngle, systemEffect * thrust, SMOOTH_THRUSTING);                    
                } else {
                    if(!accel){                        
                        //turn only, easy too.
                        thrust(weapon, turnAngle, systemEffect * thrust, SMOOTH_THRUSTING);                          
...
//                        );
                        ...
                        thrust(weapon, combinedAngle, combinedThrust*systemEffect, SMOOTH_THRUSTING/2);
